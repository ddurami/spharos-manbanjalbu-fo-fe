...
import { shippingAddressToForm } from "@/lib/address/mappers";
...
export function AddressForm({ mode, addressId }: AddressFormProps) {
  ...
  useEffect(() => {
    ...
    const address = getShippingAddressById(addressId);

    if (!address) {
      setLoadError("배송지 정보를 찾을 수 없습니다.");
      return;
    }

    setForm(shippingAddressToForm(address));
  }, [mode, addressId]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "edit" && addressId) {
      updateShippingAddress(addressId, form);
    } else {
